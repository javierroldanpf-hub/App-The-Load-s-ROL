...
export default function StaffTeamDashboard({ user, teamId, onBack, onLogout, readOnly = false }) {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "1.5rem 1.25rem 4rem", maxWidth: 640, margin: "0 auto" }}>
      ...
      {readOnly && (
        <div style={{ background: COLORS.amberDark, border: `1px solid ${COLORS.amber}`, borderRadius: 10, padding: "8px 14px", marginBottom: 14, marginTop: 44, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {/* Tabs principales */}
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: COLORS.panel, borderRadius: 12, padding: 4 }}>
        {mainTabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            flex: 1, padding: "8px 6px", borderRadius: 9, border: "none", fontSize: 11, fontWeight: 600,
      ...
      {tab === "resumen" && (
        <>
          {/* Sub-tabs de Resumen */}
          <div style={{ display: "flex", gap: 4, marginBottom: 20, background: COLORS.panelRaised, borderRadius: 10, padding: 4 }}>
            {resumenSubTabs.map((t) => (
              <button key={t.id} onClick={() => setResumenSubTab(t.id)} style={{
                flex: 1, padding: "7px 6px", borderRadius: 7, border: "none", fontSize: 11, fontWeight: 600,
